...
export async function createLead(lead: InsertLead) {
  ...
  // Fire-and-forget webhook to N8n for lead automation
  if (ENV.n8nLeadWebhookUrl && created) {
    try {
      fetch(ENV.n8nLeadWebhookUrl, {
        ...
        body: JSON.stringify({
          ...
          interestedPropertyId: created.interestedPropertyId,
          budgetMin: created.budgetMin,
          budgetMax: created.budgetMax,
          preferredNeighborhoods: created.preferredNeighborhoods,
          preferredPropertyTypes: created.preferredPropertyTypes,
          notes: created.notes,
          stage: created.stage,
          createdAt: created.createdAt,
        }),
      }).catch(err => console.error("[N8n Webhook] Lead webhook error:", err));
    } catch (error) {
      console.error("[N8n Webhook] Failed to send lead webhook:", error);
    }
  }
